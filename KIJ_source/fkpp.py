...
def solve_fvm_J(J_n, u_face, grad_u, dx, dt):
    nx = len(J_n)
    J_next = J_n.copy()
    flux = np.zeros(nx + 1)
    for j in range(1, nx):
        flux[j] = upwind_flux(J_n[j-1], J_n[j], u_face[j])
    advec = - (dt/dx)*(flux[2:-1]-flux[1:-2])
    source = dt * (J_n * 2.0 * grad_u)[1:-1]
    J_next[1:-1] = J_n[1:-1] + advec + source
    return J_next
...
def run_simulation(D=1.0, X_MIN=-10.0, X_MAX=20.0,
                   T_MAX=6.0, NX=601, NT=1601, x0_wave=0.0):
    DX = (X_MAX - X_MIN) / NX
    DT = T_MAX / (NT - 1)
    ...
    for n in range(1, NT):
        ...
        R_next = solve_fvm_R(R_current, u_face, grad_u, rJ, DX, DT)
        J_next = solve_fvm_J(J_current, u_face, grad_u, DX, DT)
        ...
        I_current, K_current, R_current, J_current = I_next, K_next, R_next, J_next
...
def plot_results(x, rho_numerical_history, rho_exact_history, time_steps,
                 X_MIN=-10.0, X_MAX=20.0, output_dir="FKPP_KIRJ"):
